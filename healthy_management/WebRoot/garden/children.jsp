    ...
    <div class="row">
        ...
        <div class="col-md-9">
           <div class="panel panel-primary">
            ...
            <%child_list = GetGardenChildren.getData(info.getUserID(), _page, page_size); %>
            <%if(child_list != null && child_list.size() > 0){ %>
            	<% for(int j = 0; j < child_list.size(); j ++ ){%>
		            ...
		            <!-- panel end -->
		    	<% } %> <!--  for 循环结束 -->
		    	<% int pages = GetGardenChildren.getPaing(info.getUserID()); %>
		    				...
		    				<nav style="text-align: center">
		            			<ul  class="pagination"">
		            				<% for(int j = 1; j <= (page_num+1); j++){ %>
		            					<%if(j == _page){ %>
		            						<li  class="active">
		            							<a href="<%=request.getContextPath() %>/garden/children.jsp?page=<%=j %>">
		            								<%=j %>
		            							</a>
		            						</li>
		            					<% }else{ %>
		            						<li><a href="<%=request.getContextPath() %>/garden/children.jsp?page=<%=j %>"><%=j %></a></li>
		            					<%} %>
		            				<%} %>
		            			</ul>
		            		</nav>
